			...
			<div class="main-page">
				<div class="sub-page">
					...
					<div class="atribute">
						...
						<p>Evaluasi Ke- <span>: <?= $pertKe ?></span></p>
					</div>
						...
						<table cellpadding="14">
							<thead>
								<tr>
									...
									<th colspan="<?= count($evaluasi) ?>">Evaluasi Ke-</th>
								...
								<tr>
									...
									foreach ($evaluasi as $row) {
										echo '<th>' . $row->evaluasi_ke . '</th>';
									} ?>
								</tr>
							</thead>
							<tbody>
								<?php foreach ($rekapEvaluasi as $value) : ?>
									<tr>
										<td><?= $value['nomor'] ?></td>
										<td><?= $value['nis'] ?></td>
										<td><?= $value['nama'] ?></td>
										<?php $evaluasi = $this->guru->getExportEvaluasi($value['idJ']);
										foreach ($evaluasi	as $val) {
											$eval = $this->guru->getNilaiEvaluasi($val->evaluasi_id, $value['nis']);
											if ($eval) {
												echo '<td>' . $eval->nilai . '</td>';
											} else {
												echo '<td>-</td>';
											}
										} ?>
